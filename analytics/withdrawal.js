...
(async () => {
  async function fetchCourse(y, s, department) {
    try {
      let now = new Date();
      let { data: res } = await axios.get(`https://gnehs.github.io/ntut-course-crawler-node/${y}/${s}/${encodeURI(department)}.json`);
      console.log(`[fetch] ${y}-${s} ${department}` + ` -- ${new Date() - now}ms`.gray);
      return res;
    } catch (e) {
  ...
  let { data: main } = await axios.get('https://gnehs.github.io/ntut-course-crawler-node/main.json');
  ...
  function calcWithdrawalRate(data, filename = '') {
    let calcResult = {};
    let totalPeople = 0, totalWithdraw = 0
    ...
    let result = Object.values(calcResult).filter(x => x.people).sort((a, b) => b.rate - a.rate).map(x => {
      x.course = x.course.map(({ name, id, courseType, people, peopleWithdraw }) => ({ name, id, courseType, people, peopleWithdraw }))
      return x
    })
    // calc quartiles
    let rates = result.sort().map(x => x.rate * 100).reverse()
  ...
  function filterPeriod(data, period, name) {
    ...
    let filteredData = data.filter(x => periodMain.includes(`${x.year}-${x.sem}`))
