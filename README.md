...
每日課程 workflow 先固定年度學期，合併三部別產物後驗證完整性、產生課綱索引，再只替換該學期的 `course/` 目錄，以移除不再被現行清單引用的舊課綱。其他學期、統計、班級與學程資料會保留。若課程或課綱擷取失敗，workflow 應失敗而非發布部分清單。所有推送 `gh-pages` 的工作共用發布佇列，避免各資料集同時推送互相覆蓋，並保留等待中的工作（`queue: max`）。
